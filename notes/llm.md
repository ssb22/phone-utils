...
* Claude: Voice output recently became available in the app even if you don’t speak the prompt (same-voice added in 2026) although it stops playing when the app is backgrounded or screen turned off (even by the screensaver). British English voices are available (no Chinese). “Memory” option added to free tier 2026 but it’s lossily reworded nightly so less useful for complex needs.
...
* Qwen Studio (previously Qwen Chat, a pun on “Gwen”, Aliyun’s 通义千问): Web version and app can access same conversations although there can be synchronisation problems and restarts under low-RAM conditions; there’s a 500-character limit to the bio and custom instructions fields under personalisation settings but cross-chat memory also available (max 50 items with automatic removal of oldest but model often reorders them): run on a simpler model with less context, but was also placed under control of the main model in early 2026 although still less inclined than Kimi to remember exactly what you ask and can temporarily fail to gain write access. Voice playback was added to both Web and app in 2025 but then got placed behind a “more” button with no autoplay (here’s a [workaround script](https://github.com/ssb22/bits-and-bobs/blob/master/chat-save-js/qwen-autoread.js) for desktop browsers); network glitches during long replies can cause voice to give up, so if you’re at the edge of your WiFi try mobile data. The “Cherry” voice with language set to Chinese also works in English if you don’t mind a bit of drama, although occasionally the app spontaneously sets language back to English thereby making any Chinese pronunciation bad.

  Model can cope with exceptionally long contexts but appears to sample at random (not directed by what’s important); slow if server is loaded; “Flash” model is easily good enough for simple tasks but needs to be set per-session. “Thinking” can be turned off on desktop Web (or in the tiny-print “desktop mode” of a mobile browser) but not on the app as of early 2026 (but the thinking is normally quick). Turning off “thinking” on desktop Web (and entering at least one prompt there to save the setting) results in it staying off in-app but not for model changes or new sessions. A workaround to suppress thinking on the app is to choose “Expand more models” and select an older model that’s less inclined to do it: the non-Max Qwen3 models, i.e. the Qwen3-235B-A22B variants, Coder or Omni-Flash which I think is 30B-A3B (don’t ask Omni-Flash to talk you to sleep: it’s likely to get stuck in an infinite loop that can’t be voiced), and the Qwen2.5-Max 325B with unpublished active count, can skip “thinking” themselves, but none of the 3.5 models seem able to self-skip it.

  Voice input option puts the transcribed text into the editor so you can correct it before sending, or even just use as alternative voice dictation software, but you can’t use it to add to existing text (all text is cleared when microphone button is pressed) and it can be cleared when screenreader activates unless cancelled quickly. Separate voice-call mode does not read the memories.
